...
monty_hall <- function() {
  ...
  results <- c(switch = doors_remain[-door_host] %>% as.integer,
...
results <- replicate(n = 50000, monty_hall()) %>% t %>% 
  data.frame %>% 
...
results %>% 
  ...
  geom_line() +
  labs(title = "Cumulative Wins Under Various Monty Hall Strategies",
